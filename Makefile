...
# ──── Variables ────────────────────────────────────────────────────────────
# Prefer fvm-managed Flutter/Dart if fvm is installed; fall back to system.
...
.stamps/check-deps:
	...
	@(command -v fvm >/dev/null && fvm flutter --version >/dev/null 2>&1) || \
	 command -v flutter >/dev/null || \
	 (echo "ERROR: neither fvm nor flutter found. Install fvm: https://fvm.app/documentation/getting-started/installation" && exit 1)
...
# Docker image for reproducible Android builds.
DOCKER_IMAGE := ghcr.io/foggedlens/deflock-builder:latest
...
.PHONY: all check analyze test ci clean help version \
        ...
        docker-ci docker-build-apk-debug docker-build-apk-release \
        docker-build-aab-release docker-build-image
...
docker-ci:
	docker run --rm -v $(CURDIR):/app -w /app $(DOCKER_IMAGE) make ci

docker-build-apk-debug:
	docker run --rm -v $(CURDIR):/app -w /app $(DOCKER_IMAGE) make build-apk-debug

docker-build-apk-release:
	docker run --rm -v $(CURDIR):/app -w /app $(DOCKER_IMAGE) make release-apk FLUTTER_BUILD_ARGS="$(FLUTTER_BUILD_ARGS)"

docker-build-aab-release:
	docker run --rm -v $(CURDIR):/app -w /app $(DOCKER_IMAGE) make release-aab FLUTTER_BUILD_ARGS="$(FLUTTER_BUILD_ARGS)"
...
help:
	...
	@echo ""
	@echo "Docker (reproducible builds in container)"
	@echo "  docker-ci                Run analyze + test in Docker"
	@echo "  docker-build-apk-debug   Build debug APK in Docker"
	@echo "  docker-build-apk-release Build release APK in Docker"
	@echo "  docker-build-aab-release Build release AAB in Docker"
	@echo "  docker-build-image       Build the Docker image locally"
	@echo ""
	@echo "Housekeeping"
